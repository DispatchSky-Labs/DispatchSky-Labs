...
### Prerequisites
- ✅ Xcode 14.0 or later (preferably Xcode 15+)
- ✅ macOS (for Xcode)
- ✅ Apple ID (free account works for simulator, paid account needed for device)
...
2. **Interface Options**
   ...
   - **Storage:** Select **SwiftData** ⚠️ **CRITICAL!** (Not "None" or "Core Data")
...
2. **Delete Them**
   - Right-click on `ContentView.swift` → **Delete** → Choose **Move to Trash**
   - Right-click on `CieloTrackerProApp.swift` → **Delete** → Choose **Move to Trash**
...
2. **Add Files to Xcode**
   ...
   - Drop them in the appropriate groups:
     - `Flight.swift`, `WeatherData.swift` → `Models/`
     - `WeatherService.swift`, `FlightSyncService.swift` → `Services/`
     - All view files → `Views/`
     - `FlightListViewModel.swift` → `ViewModels/`
     - `CieloTrackerApp.swift` → Root level (same level as `CieloTrackerPro` folder)
...
3. **Configure When Adding**
   ...
   - ✅ **Check:** Your target (`CieloTrackerPro`) in "Add to targets"
   - Click **Finish**
...
2. **Check Target Membership**
   - Select any file
   - Open File Inspector (right sidebar, or press ⌘⌥1)
...
3. **Build Project**
   ...
   - Or click the play button (▶️) to build and run

4. **Check for Errors**
   - Look at the bottom of Xcode for any error messages
   - Common issues:
     - Missing files → Check target membership
     - SwiftData errors → Verify iOS 15.0+ deployment
     - Import errors → Check all files are added
...
2. **Import to iOS App**
   ...
   - Tap the menu button (⋯) in top right
   - Tap **"Sync"**
...
- **Fix:** Select the file → File Inspector → Check "Target Membership"
...
4. ✅ Test adding flights manually
5. ⬜ (Optional) Enable iCloud sync
6. ⬜ (Optional) Test on physical device
7. ⬜ (Optional) Add to App Store
...
If you see the app running in the simulator with "CieloTracker Pro" in the title bar, you're all set! 🎉
